    ...
    while (bytecode) {
        ...
        switch (bytecode->instruction) {
            ...
            case vm_instr_loop_start:
                ...
                memcpy(native->code_as_data + current_offset + 0x2, nint_buffer, sizeof(nint));
                current_offset += g_machine_code.loop_begin_code_len;
                loop_begin_address = current_offset;
                break;
            case vm_instr_loop_end:
                // potential overflow (uintptr_t -> nint) bug
                address_offset = (nint)loop_begin_address - (nint)(current_offset + 0x0d);
                ...
                memcpy(native->code_as_data + current_offset + 0x9, nint_buffer, sizeof(nint));
